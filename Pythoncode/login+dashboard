...
# functie die ervoor zorgt dat het makkelijker is om tussen talen te switchen
def t(key):
...
def login_screen():
    ...
    with st.form("login_form"):
        ...
        with col2:
            login_button = st.form_submit_button(t('li inloggen'), width="stretch")
...
def main_app():
    ...
    with col1:
        ...
        with st.expander(t('mp filter'), expanded=False):
            ...
            var_keys = pd.concat([
                talen.loc["ex sdnn_1":"ex HR Diff"],
                talen.loc["ex bd gem 1":"ex bmi gem 2"]
            ]).index.tolist()
            ...
            meest_voorkomende_categorieën = st.multiselect(
                t('mp mvc'),
                options=mvc_keys,
    ...
    with col2:
        ...
        # pagina 2 = grafieken
            ...
            if len(st.session_state.variabelen) > 0:
                ...
                grafiek_cols = st.columns(len(st.session_state.variabelen))
                ...
                for i, variabele_key in enumerate(st.session_state.variabelen):
                    ...
                    fig.update_layout(
                        xaxis_title=t('ta wijk'),
                        yaxis_title=talen.loc[variabele_key, st.session_state.taal],
                        legend_title=t('ta wijk')
                    )
        ...
        # pagina 3 = tabellen
            ...
            if len(st.session_state.mvc) > 0:
                ...
                nieuwe_kolomnamen = (
                    [t('ta wijk')] +
                    [talen.loc[mvc, st.session_state.taal] for mvc in st.session_state.mvc if mvc in mapping]
                )
    ...
    # Uitloggen helemaal onderaan de dashboard
    st.markdown("<div style='height: 3rem'></div>", unsafe_allow_html=True)

    logout_col1, logout_col2, logout_col3 = st.columns([4, 2, 4])

    with logout_col2:
        if st.button(t('li lu'), width="stretch"):
            st.session_state.logged_in = False
            st.rerun()
